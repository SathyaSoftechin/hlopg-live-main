...
function Home() {
        ...
        if (res.data.success && Array.isArray(res.data.hostels)) {
          const processedHostels = res.data.hostels.map((hostel) => {
            ...
            if (hostel.images && Array.isArray(hostel.images)) {
              ...
              images = [getFullImageUrl(hostel.image)];
            } else {
              images = [defaultPGImg];
            }
    ...
    const fetchLiked = async () => {
      try {
        ...
        if (res.data.success && Array.isArray(res.data.data)) {
          ...
          localStorage.setItem("hlopgLikedHostels", JSON.stringify(likedIds));
        } else {
          setLikedPgIds([]);
        }
      } catch (err) {
  ...
  /* ---------------- Update localStorage liked hostels ---------------- */
  const updateLocalStorageLiked = (hostelId, liked) => {
    try {
      ...
      if (liked) {
        if (!likedHostels.includes(hostelId)) likedHostels.push(hostelId);
      } else {
    ...
    try {
      ...
      if (res.data.success) {
        if (res.data.liked === true) {
          setLikedPgIds((prev) => {
            if (!prev.includes(pg.id)) return [...prev, pg.id];
            return prev;
          });
